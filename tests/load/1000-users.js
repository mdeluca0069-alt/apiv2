/**
 * 1000-users.js — Tier 4 (Stress): 1000 concurrent traders
 *
 * Full certification thresholds — same as Tier 3 (500 users):
 *   Order P95 < 3000ms
 *   Close P95 < 4000ms
 *   HTTP 5xx = 0  (zero tolerance)
 *   Double settlement = 0
 *
 * A PASS at 1000 users establishes the maximum certified concurrent capacity.
 */
...
const orderLatency     = new Trend("order_latency_ms_1000",  true);
const closeLatency     = new Trend("close_latency_ms_1000",  true);
const walletLatency    = new Trend("wallet_latency_ms_1000", true);
const filledOrders     = new Counter("orders_filled_1000");
const rejectedOrders   = new Counter("orders_rejected_1000");
const settledPositions = new Counter("positions_settled_1000");
const doubleSettlement = new Counter("double_settlement_1000");
const swapDuplicates   = new Counter("swap_duplicates_1000");
const http5xxCount     = new Counter("http_5xx_count_1000");
const httpErrors       = new Rate("http_errors_1000");
...
export const options = {
  ...
  thresholds: {
    order_latency_ms_1000:  ["p(95)<3000", "p(99)<8000"],
    close_latency_ms_1000:  ["p(95)<4000"],
    http_errors_1000:       ["rate<0.005"],
    http_5xx_count_1000:    ["count<1"],
    double_settlement_1000: ["count<1"],
  },
};
...
export default function () {
  ...
  const clientId = `1000-${padId(idx)}-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;
...
export function handleSummary(data) {
  ...
  const p95  = val("order_latency_ms_1000", "p(95)");
  const p99  = val("order_latency_ms_1000", "p(99)");
  const p95c = val("close_latency_ms_1000", "p(95)");
  const p95w = val("wallet_latency_ms_1000", "p(95)");
  ...
  const results = {
    ...
    close_p95:         p95c,
    wallet_p95:        p95w,
    orders_filled:     cnt("orders_filled_1000"),
    orders_rejected:   cnt("orders_rejected_1000"),
    positions_settled: cnt("positions_settled_1000"),
    double_settlement: cnt("double_settlement_1000"),
    swap_duplicates:   cnt("swap_duplicates_1000"),
    http_5xx_count:    cnt("http_5xx_count_1000"),
    http_error_rate:   rate("http_errors_1000"),
  ...
  return {
    stdout: `
╔══════════════════════════════════════════════════════════╗
║     TIER 4: 1000 USERS — STRESS TEST RESULTS            ║
╚══════════════════════════════════════════════════════════╝
...
`,
    "k6-results-1000.json": JSON.stringify(results, null, 2),
  };
}
