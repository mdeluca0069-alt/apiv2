/**
 * 500-users.js — Tier 3 (Certification): 500 concurrent traders
 *
 * GO/NO-GO thresholds (ALL must PASS for FULL PRODUCTION READY):
 *   Order P95 < 3000ms
 *   Close P95 < 4000ms
 ...
 *   Swap duplicates = 0
 *
 * Outputs k6-results-500.json for certification-runner.ps1 to parse.
 */
...
const orderLatency     = new Trend("order_latency_ms_500",  true);
const closeLatency     = new Trend("close_latency_ms_500",  true);
const walletLatency    = new Trend("wallet_latency_ms_500", true);
const filledOrders     = new Counter("orders_filled_500");
const rejectedOrders   = new Counter("orders_rejected_500");
const settledPositions = new Counter("positions_settled_500");
const doubleSettlement = new Counter("double_settlement_500");
const swapDuplicates   = new Counter("swap_duplicates_500");
const http5xxCount     = new Counter("http_5xx_count_500");
const httpErrors       = new Rate("http_errors_500");
...
export const options = {
  ...
  thresholds: {
    // Certification GO/NO-GO criteria
    order_latency_ms_500:  ["p(95)<3000", "p(99)<6000", "p(50)<1000"],
    close_latency_ms_500:  ["p(95)<4000"],
    http_errors_500:       ["rate<0.001"],
    http_5xx_count_500:    ["count<1"],    // zero 5xx tolerance
    double_settlement_500: ["count<1"],    // zero double-settlement tolerance
  },
};
...
export default function () {
  ...
  const clientId = `500-${padId(idx)}-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;
  ...
  // ── Close the specific position (or fetch+close all) ────────────────────────
  if (positionId) {
    const tc = Date.now();
...
export function handleSummary(data) {
  ...
  const results = {
    ...
    order_p50:         val("order_latency_ms_500", "p(50)"),
    order_p95:         val("order_latency_ms_500", "p(95)"),
    order_p99:         val("order_latency_ms_500", "p(99)"),
    close_p95:         val("close_latency_ms_500", "p(95)"),
    wallet_p95:        val("wallet_latency_ms_500", "p(95)"),
    orders_filled:     cnt("orders_filled_500"),
    orders_rejected:   cnt("orders_rejected_500"),
    positions_settled: cnt("positions_settled_500"),
    double_settlement: cnt("double_settlement_500"),
    swap_duplicates:   cnt("swap_duplicates_500"),
    http_5xx_count:    cnt("http_5xx_count_500"),
    http_error_rate:   rate("http_errors_500"),
  ...
  return {
    stdout: `
╔══════════════════════════════════════════════════════════╗
║     TIER 3: 500 USERS — CERTIFICATION RESULTS           ║
╚══════════════════════════════════════════════════════════╝
...
THROUGHPUT
  ...
  Throughput:        ${dur > 0 ? (results.total_requests / dur).toFixed(1) : "N/A"} req/s
`,
    "k6-results-500.json": JSON.stringify(results, null, 2),
  };
}
