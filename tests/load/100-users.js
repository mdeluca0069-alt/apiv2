/**
 * 100-users.js — Tier 1: 100 concurrent traders
 *
 * Certification thresholds (must all PASS for GO):
 *   Order P95 < 3000ms, Close P95 < 4000ms, HTTP 5xx = 0
 *
 * Outputs k6-results-100.json for certification-runner.ps1 to parse.
 */
...
const swapDuplicates  = new Counter("swap_duplicates");
...
export const options = {
  ...
  thresholds: {
    order_latency_ms: ["p(50)<500",  "p(95)<3000", "p(99)<4000"],
    close_latency_ms: ["p(95)<4000"],
    http_errors:      ["rate<0.001"],
    http_5xx_count:   ["count<1"],
  },
};
...
export default function () {
  ...
  const clientId = `100-${padId(idx)}-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;

  const t0 = Date.now();
  const orderRes = http.post(`${BASE_URL}/api/v1/trading/order`, JSON.stringify({
    symbol, side, type: "MARKET", quantity: 1000, leverage: 10, clientOrderId: clientId,
  }), { headers: auth });
  orderLatency.add(Date.now() - t0);
  if (orderRes.status >= 500) { http5xxCount.add(1); errorRate.add(1); }
...
export function handleSummary(data) {
  ...
  const results = {
    ...
    swap_duplicates:   cnt("swap_duplicates"),
  ...
  return {
    stdout: `
=== TIER 1: 100 USERS — MEASURED RESULTS ===
Order  P50:              ${fmt(p50)}
...
`,
    "k6-results-100.json": JSON.stringify(results, null, 2),
  };
}
